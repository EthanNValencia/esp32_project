...
static void wifi_app_default_wifi_init(void) {
	...
	// Note: Default wifi config operations must be in order!
...
/**
 * Configures the access point settings and assigns the static ip to the soft access point.
 */
static void wifi_app_soft_ap_config(void) {
	...
	// Start ap dhcp server for connecting to devices.
...
/**
 * Initializes the wifi application event handler for wifi and ip related events.
 */
 ...
 * @param pvParameters which can be used to pass parameters to the task.
 */
static void wifi_app_task(void *pvParameters) {
	wifi_app_queue_message_t msg;
	wifi_app_event_handler_init(); // Init event handler.
	wifi_app_default_wifi_init(); // Init TCP/IP stack and wifi config.
	wifi_app_soft_ap_config(); // Soft access point config.
	ESP_ERROR_CHECK(esp_wifi_start()); // Start wifi and error check.
	wifi_app_send_message(WIFI_APP_MSG_START_HTTP_SERVER); // Send first event message.
	for(;;) {
...
void wifi_app_start(void) {
	...
	wifi_app_queue_handle = xQueueCreate(3, sizeof(wifi_app_queue_message_t)); // creates message queue
